...
int main(void)
{
    ...
    PCP_LOG(PCP_LOGLVL_NONE, "big msg %s %s %s %s %s %s %s", big_string, big_string, big_string, big_string, big_string, big_string, big_string);

    pcp_set_loggerfn(test_logfn);
    PCP_LOG(PCP_LOGLVL_NONE, "%s", "none");
    TEST((PCP_LOGLVL_INFO<=PCP_MAX_LOG_LEVEL) ||((test_mode==PCP_LOGLVL_NONE)&&(strcmp("none",test_msg)==0)));
    PCP_LOG(PCP_LOGLVL_ERR, "%s", "error");
    TEST((PCP_LOGLVL_INFO<=PCP_MAX_LOG_LEVEL) ||((test_mode==PCP_LOGLVL_ERR)&&(strcmp("error",test_msg)==0)));
    PCP_LOG(PCP_LOGLVL_WARN, "%s", "warn");
    TEST((PCP_LOGLVL_INFO<=PCP_MAX_LOG_LEVEL) ||((test_mode==PCP_LOGLVL_WARN)&&(strcmp("warn",test_msg)==0)));
    PCP_LOG(PCP_LOGLVL_INFO, "%s", "info");
    TEST((PCP_LOGLVL_INFO<=PCP_MAX_LOG_LEVEL) ||( (test_mode==PCP_LOGLVL_INFO)&&(strcmp("info",test_msg)==0)));
    PCP_LOG(PCP_LOGLVL_PERR, "%s", "perr");
    TEST((PCP_LOGLVL_INFO<=PCP_MAX_LOG_LEVEL) ||((test_mode==PCP_LOGLVL_PERR)&&(strcmp("perr",test_msg)==0)));
    PCP_LOG(PCP_LOGLVL_DEBUG, "%s", "debug");
    TEST((PCP_LOGLVL_INFO<=PCP_MAX_LOG_LEVEL) ||((test_mode==PCP_LOGLVL_DEBUG)&&(strcmp("debug   ",test_msg)==0)));
